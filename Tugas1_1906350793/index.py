...
class InvertedIndexReader(InvertedIndex):
    ...
    def __next__(self):
        ...
        """
        if self.pointer < len(self.postings_dict.keys()):
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list):
        ...
        if len(self.terms) == 0:
            ...
            start_position_in_index_file = term_tuple[0] + term_tuple[2]
  
        """Add term ke self.terms"""
        self.terms.append(term)

        """Encode postings_list"""
        hasil_encode = self.postings_encoding.encode(postings_list)

        """Menyimpan metadata"""
        number_of_postings_in_list = len(postings_list)
        length_in_bytes_of_postings_list = len(hasil_encode)
        ...
        self.index_file.write(hasil_encode)

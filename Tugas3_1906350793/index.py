...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        """
        term_tuple = self.postings_dict[term]

        self.index_file.seek(term_tuple[0])
        postings_list = self.index_file.read(term_tuple[2])
        postings_list = self.postings_encoding.decode(postings_list)

        self.index_file.seek(term_tuple[0]+term_tuple[2])
        tf_list = self.index_file.read(term_tuple[3])
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        if len(self.terms) == 0:
            ...
            previous_term = self.terms[-1]
            term_tuple = self.postings_dict[previous_term]
            start_position_in_index_file = term_tuple[0] + term_tuple[2] + term_tuple[3]
  
        """Add term ke self.terms"""
        self.terms.append(term)

        """Update self.doc_length"""
        for pointer in range(len(postings_list)):
            self.doc_length[postings_list[pointer]] =  self.doc_length.get(postings_list[pointer], 0) + tf_list[pointer]

        """Encode postings_list"""
        hasil_encode_pe = self.postings_encoding.encode(postings_list)
        hasil_encode_tf = self.postings_encoding.encode_tf(tf_list)

        """Menyimpan metadata"""
        number_of_postings_in_list = len(postings_list)
        length_in_bytes_of_postings_list = len(hasil_encode_pe)
        length_in_bytes_of_tf_list = len(hasil_encode_tf)
        self.postings_dict[term] = (start_position_in_index_file, number_of_postings_in_list, length_in_bytes_of_postings_list, length_in_bytes_of_tf_list)

        """Append ke index file"""
        self.index_file.write(hasil_encode_pe)
        self.index_file.write(hasil_encode_tf)

...
def dcg(ranking):
  ...
  for i in range(1, len(ranking) + 1):
    pos = i - 1
    score += ranking[pos] / math.log(i + 1, 2)
  return score
...
def eval(qrels, query_file = "queries.txt", k = 2):
  """ 
  ...
  with open(query_file) as file:
    ...
    for qline in file:
      ...
      for (score, doc) in BSBI_instance.retrieve_bm25(query, k = k):
        ...
        for (docL, score) in sorted_did_scores: 
          did = int(re.search(r'.*\\(.*)\.txt', docL).group(1))
